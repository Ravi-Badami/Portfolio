...
const Hero = () => {
  ...
  return (
    <section id="home" className="pt-24 pb-12 md:pt-32 md:pb-16 px-6 max-w-7xl mx-auto min-h-screen flex flex-col justify-center">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        
        {/* Cell 1: Intro Text (Span 1 on mobile, 2 on desktop) */}
        <div className="col-span-1 md:col-span-2 bg-white rounded-3xl p-6 md:p-12 border border-zinc-200 flex flex-col justify-center shadow-sm">
          <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-600 mb-6 font-display tracking-tight leading-tight">
            {heroData.intro}
          </h1>
          <p className="text-lg md:text-xl lg:text-2xl text-zinc-500 max-w-lg leading-relaxed">
            {heroData.description}
          </p>
        </div>

        {/* Cell 2: Profile Photo */}
        <div className="relative group overflow-hidden rounded-3xl border border-zinc-200 aspect-square md:aspect-auto order-first md:order-none">
        ...
        <div className="col-span-1 bg-zinc-900 rounded-3xl p-5 md:p-8 border border-zinc-800 flex flex-col justify-between group overflow-hidden relative min-h-[300px] md:min-h-0">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2 group mt-auto">
                <a 
                  href={`mailto:${contactData.email}`} 
                  className="flex-1 flex items-center gap-2 p-2 bg-zinc-800 rounded-lg text-white hover:bg-zinc-700 transition-colors border border-zinc-700 hover:border-zinc-600 overflow-hidden"
                >
                  <svg className="w-5 h-5 text-zinc-400 group-hover:text-white transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <span className="text-zinc-300 text-sm font-medium tracking-wide group-hover:text-white transition-colors truncate">
                    {contactData.email}
                  </span>
                </a>
                <button 
                  onClick={handleCopyEmail}
                  className="p-2 bg-zinc-800 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-700 transition-colors border border-zinc-700 hover:border-zinc-600 relative flex-shrink-0"
        ...
        {/* Cell 4: Tech Stack */}
        <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-zinc-50 to-zinc-100 rounded-3xl p-6 md:p-10 border border-zinc-200 flex flex-col justify-between overflow-hidden relative min-h-[250px] md:min-h-0">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 relative z-10">
             {['JavaScript', 'React', 'Flutter', 'Node.js', 'Express.js', 'Tailwind CSS', 'MongoDB', 'Java/Quarkus', 'Supabase'].map((tech) => (
                <div key={tech} className="bg-white px-3 py-2 md:px-5 md:py-3 rounded-2xl border border-zinc-200 shadow-sm hover:shadow-md hover:border-zinc-300 transition-all duration-300 cursor-default">
                  <span className="font-semibold text-xs md:text-base text-zinc-700">{tech}</span>
                </div>
             ))}
          </div>
        </div>
